...
Usage:
    python3 CorsOne_professional.py -u https://example.com
    python3 CorsOne_professional.py -l targets.txt -w 10
    cat domains.txt | python3 CorsOne_professional.py -w 20
...
class CORSVulnerabilityScanner:
    ...
    def save_results(self):
        ...
        try:
            ...
            file_suffix = output_path.suffix.lower().lstrip('.')
            if file_suffix in ['json', 'txt']:
                output_format = file_suffix
            ...
            if output_format == 'json':
                ...
                # Save text results
                with open(final_path, 'w') as f:
                    if self.config.stop_on_first and self.vulnerable_results:
                        f.write(str(self.vulnerable_results[0]) + '\n')
                    else:
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-f', '--format', choices=['txt', 'json'], default='txt',
                       help='Output format (txt or json). Default: txt. If file extension differs, extension takes precedence.')
